...
Une fois écartées ces deux catégories, **nous obtenons une liste de 136 entreprises soumises au droit commercial ayant employé plus de 5000 salariés deux années de suite**.

---

### Recoupements

En croisant les identifiants uniques Siren des entreprises répondant aux critères Devoir de vigilance pour les bases Greffe et Sirene, il apparaît que **l'intersection des listes se limite à 45 unités légales**. Aucune des bases ne peut donc être considérée comme suffisante à établir une liste.
...
[La base de données Orbis](https://www.bvdinfo.com/fr-fr/our-products/company-information/international-products/orbis) est un service fourni par la société Bureau van Dijk (filiale de la société d'analyse financière Moody's).

Elle propose aux acteurs privés et publics des données micro-économiques sur 300 millions d'entreprises dans le monde, notamment leurs effectifs. Parmi les critères de recherche, elle offre notamment la possibilité de retracer les liens de propriété financière entre les établissements d'une même entreprise à l'internationale, offrant ainsi une estimation du périmètre réel des groupes.
...
1. Plus de 5000 salariés France
...
2. Plus de 10000 salariés mondialement
...
L'intersection des deux listes par nom renvoie 145 établissements, indiquant que la quasi totalité des entreprises dépassant le seuil d'emploi à l'international (147) le dépassent déjà en France. 114 entreprises de la liste française n'apparaissent pas dans la liste internationale, soit plus du double de la liste complète établie via le greffe.

---

### Croisement des listes par nom

.pull-left[Nous établissons un croisement par nom entre la base Orbis pour le critère français et les deux autres bases. **Seules 31 noms d'entreprises** se retrouvent dans les trois bases.

**Rappel méthodologique** : les recoupements sont réalisés sur les noms et non sur les identifiants uniques siren(non disponibles sur Orbis). Ils sont donc moins précis (d'où la valeur d'intersection de 33 entre Sirene et Greffe au lieu de 45 obtenus précédemment). Ces comparaisons sont donc à utiliser avec réserve comme des calculs *a minima*.]
...
Sur la base de données ouvertes, les associations CCFD Terres solidaires et Sherpa ont établi une liste des entreprises ayant déclarées être soumises à la loi Devoir de vigilance ou ayant déjà publié un plan de ce type.
...
## Liste établie via les données en open data
